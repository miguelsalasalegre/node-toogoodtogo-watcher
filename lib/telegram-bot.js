...
function createBot() {
  console.log(process.env["TOOGOODTOGO_CONFIG"]);
  config.clear();
  config.set(process.env["TOOGOODTOGO_CONFIG"]);
  console.log("Initializing bot creation");
  const botToken = getBotToken();
  if (!isEnabled() || !botToken) {
    console.log("Telegram is not enabled or token is not provided");
    return null;
  }
...
function isEnabled() {
  ...
  console.log(
    `Is Telegram enabled?: ${config.get("notifications.telegram.enabled")}`
  );
...
function getBotToken() {
  console.log(
    `Telegram token?: ${config.get("notifications.telegram.botToken")}`
  );
